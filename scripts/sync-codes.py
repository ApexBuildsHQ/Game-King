...
def extract_relevant_sections(text):
    ...
    return extracted[:6000] if extracted else text[:6000]

def extract_codes_with_gemini(prompt):
    # 1. الاستدعاء المباشر لـ Gemini API
    if GEMINI_API_KEY:
        try:
            ...
            if res.status_code == 200:
                ...
                print(f"❌ Gemini Direct HTTP Error {res.status_code}: {res.text[:120]}")
        except Exception as e:
            print(f"❌ Gemini Direct Exception: {e}")

    # 2. الاستدعاء الاحتياطي عبر Cloudflare Worker (Gemini)
    if CF_WORKER_URL and CF_WORKER_SECRET:
        try:
            ...
            if res.status_code == 200:
                ...
                print(f"❌ Worker AI HTTP Error {res.status_code}: {res.text[:120]}")
        except Exception as e:
...
def main():
    ...
    print(f"🚀 Processing {len(GAMES_CONFIG)} games using Gemini Flash...")
    ...
    for index, game in enumerate(GAMES_CONFIG, start=1):
        ...
        prompt = (
            f"Extract active redeem/gift codes for game ID '{game['id']}' from this text.\n"
            'Return valid JSON format: {"' + game['id'] + '": [{"code": "XYZ", "reward": "100 Gems"}]}\n'
            f"Text:\n{relevant_text}"
        )
        
        print(f"[{index}/{len(GAMES_CONFIG)}] 🤖 Processing {game['name']} via Gemini...")
        extracted_json = extract_codes_with_gemini(prompt)
        
        if extracted_json:
            codes_db = merge_and_save_codes(codes_db, extracted_json)
            hashes_db[game['id']] = current_hash
            print(f"[{index}/{len(GAMES_CONFIG)}] ✅ Extracted codes for {game['name']}")
        else:
