...
def parse_json_from_text(text):
    ...
    # تنظيف وسوم الماركداون والشارحات
...
def main():
    ...
    for batch_index, batch in enumerate(game_batches):
        ...
        for game in batch:
            ...
            pending_hashes[game['id']] = current_hash
            batch_prompt_text += f"\n--- GAME: {game['id']} ({game['name']}) ---\n{raw_text[:8000]}\n"

        if pending_hashes and batch_prompt_text.strip():
            prompt = (
            ...
            if extracted_json:
                ...
                hashes_db.update(pending_hashes)
