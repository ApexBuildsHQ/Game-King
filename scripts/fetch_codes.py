...
def fetch_raw_data(game_url):
    ...
    # المحاولة الثانية: طلب عبر Cloudflare Worker في حال الفشل أو الحظر
    ...
    # معالجة وتنظيف Fandom API من وسوم HTML واستخراج النص المباشر
...
def extract_codes_with_ai_fallback(prompt):
    # 1. Groq API
    if GROQ_API_KEY:
        try:
    ...
    if CF_WORKER_URL and CF_WORKER_SECRET:
        try:
            ...
            if res.status_code == 200:
                ...
                return parse_json_from_text(str(content))
        except Exception as e:
            print(f"❌ Worker AI failed: {e}")
...
def merge_and_save_codes(existing_db, new_data):
    for game_id, codes in new_data.items():
        ...
        existing_strings = {c['code'] for c in current_codes}
        
        updated_list = list(current_codes)
        for item in codes:
            code_str = item.get('code', '').strip().upper()
            if code_str and code_str not in existing_strings:
                updated_list.insert(0, {
                    "code": code_str,
                    "reward": item.get('reward', 'Free Rewards'),
...
def main():
    ...
    batch_size = 3
    game_batches = [GAMES_CONFIG[i:i + batch_size] for i in range(0, len(GAMES_CONFIG), batch_size)]

    print(f"🚀 Processing {len(GAMES_CONFIG)} games across {len(game_batches)} batches...")
    ...
    for batch_index, batch in enumerate(game_batches):
        ...
        for game in batch:
            ...
            if hashes_db.get(game['id']) == current_hash:
                print(f"⚡ Skipping {game['name']} (No changes detected)")
                continue
        ...
        if batch_has_updates and batch_prompt_text.strip():
            prompt = (
                "Extract active redeem/gift codes from the following text for each game.\n"
                "Return ONLY a strict JSON object mapping game_id to an array of objects format: "
                '{"game_id": [{"code": "ABC", "reward": "100 Gems"}]}\n'
                f"Text:\n{batch_prompt_text}"
            )
            
            print(f"🤖 Processing Batch {batch_index + 1}/{len(game_batches)} via AI...")
            extracted_json = extract_codes_with_ai_fallback(prompt)
            if extracted_json:
                codes_db = merge_and_save_codes(codes_db, extracted_json)

            if batch_index < len(game_batches) - 1:
                print(f"⏳ Sleeping {DELAY_BETWEEN_BATCHES}s...")
