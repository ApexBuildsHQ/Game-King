...
def process():
    ...
    for game in games:
        ...
        try:
            # 1. تنزيل الملف بأقصى سرعة باستخدام aria2c
            ...
            subprocess.run(f"cd '{EXTRACT_DIR}' && 7z a -t7z -m0=lzma2 -mx=3 '../../{obfuscated_filename}' ./*", shell=True, check=True)

            # 5. الرفع إلى Hugging Face
            api.upload_file(
            ...
            # 6. توليد الرابط المباشر وتحديث progress.json
